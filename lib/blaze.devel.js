...
Blaze.loadTemplate = function(name, path, callback) {
  var url;
  if (path == null) {
    path = ".";
  }
  ...
  if (typeof Template[name] === "undefined") {
    ...
    return $.get(url).done((function(_this) {
      return function(source) {
        Blaze.compileTemplate(name, source);
        $.holdReady(false);
...
Blaze.compileTemplate = function (name, source) {
  var renderFuncCode = SpacebarsCompiler.compile(source, {isTemplate: true});
...
Blaze.init = function(){
  $("script[type='text/spacebars']").each(function(index, script){
    Blaze.compileTemplate( script.getAttribute('name'), script.innerHTML );
  });
  ...
  $(document).ready(function () { 
    ...
    if (Template.main) {
      ...
      $(document).trigger("blazeReady")
    }else{
      Blaze.loadTemplate( "main.tmp", ".", function() {
        $(document).trigger("blazeReady")
      })
    }
  });
